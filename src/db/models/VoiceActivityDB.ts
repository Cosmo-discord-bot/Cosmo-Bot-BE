...
import {
    IGuildVoiceActivity,
    IVoiceActivity,
} from '../../interfaces/statistics/IVoiceActivity'
...
export class VoiceActivityDB {
    ...
    public async insertVoiceActivity(
        guildId: string,
        event: IVoiceActivity
    ): Promise<void> {
        try {
            await this.model.updateOne(
                { guildId },
                { $push: { activities: event } },
                { upsert: true }
            )
            logger.info(
                `insertVoiceActivity: Event inserted - ${guildId} - ${event.userId} - ${event.channelId}`
            )
        } catch (error) {
            logger.error(
                `insertVoiceActivity: Error inserting event - ${guildId}, Error: ${error}`
            )
        }
    }

    public async onLeaveUpdateVoiceActivity(
        guildId: string,
        userId: string,
        channelId: string
    ): Promise<void> {
        try {
            ...
            if (result.matchedCount === 0) {
                ...
                logger.info(
                    `updateLeaveVoiceActivity: Leave event updated - ${guildId} - ${userId} - ${channelId}`
                )
            }
        } catch (error) {
            logger.error(
                `updateLeaveVoiceActivity: Error updating leave event - ${guildId}, Error: ${error}`
            )
        }
    }
}
